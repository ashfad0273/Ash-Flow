...
import { Upload, Download, Eye, Layers } from 'lucide-react';
  ...
  const handleUpload = async (e: any) => {
    ...
    for (const file of rawFiles) {
      ...
      for (const format of selectedFormats) {
        const compressed = await ashCompressEngine(file, format, 0.9);
        allProcessed.push({
  ...
  return (
    <main className="min-h-screen bg-[#080808] text-white p-8 md:p-20 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Format Selection Bento */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {formats.map(f => (
            <button 
              key={f}
              onClick={() => setSelectedFormats(prev => prev.includes(f) ? prev.filter(x => x !== f) : [...prev, f])}
              className={`p-6 rounded-3xl border transition-all ${selectedFormats.includes(f) ? 'bg-white text-black border-white' : 'bg-white/5 border-white/10 text-gray-500'}`}
            >
        ...
        <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-white/10 bg-white/[0.02] rounded-[2.5rem] cursor-pointer hover:bg-white/[0.05] transition-all mb-12">
          ...
          <p className="text-gray-400 text-sm tracking-wide">Select images for {selectedFormats.join(' + ')}</p>
            ...
            <div key={i} className="flex items-center justify-between p-6 bg-white/[0.03] border border-white/5 rounded-[2rem] backdrop-blur-xl">
              ...
              <div className="flex items-center gap-8">
                <div className="text-right">
                  ...
                  <p className="text-green-400 text-sm font-bold">-{Math.round(100 - (res.compressedSize / res.originalSize * 100))}%</p>
                </div>
